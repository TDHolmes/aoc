...
//! on the ferry, so the docking parameters aren't being correctly initialized in the
...
//!
//! The initialization program (your puzzle input) can either update the bitmask or write
//! a value to memory. Values and memory addresses are both 36-bit unsigned integers.
...
mod test {
    ...
    const EXAMPLE_DATA: &str = "mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X
mem[8] = 11
mem[7] = 101
mem[8] = 0";
